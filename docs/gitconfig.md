...
Shared Git configuration, tracked at `git/.gitconfig` and symlinked to `$HOME/.gitconfig` on **both** Windows (by `setup-windows.ps1`) and WSL (by `wsl/setup-wsl.sh`, via `setup-wsl.ps1` passing `--repo-path`). Editing this one file changes Git behavior identically in both environments — no need to update two config blocks.
...
Running `setup-wsl.sh` standalone (without `setup-wsl.ps1`, e.g. `bash wsl/setup-wsl.sh` inside an already-provisioned WSL) falls back to applying the same settings inline via `git config --global`, since there's no repo path to symlink to in that case.
